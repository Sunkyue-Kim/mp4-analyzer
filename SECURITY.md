...
Standalone Web Media Analyzer parses untrusted local or remote media bytes in the browser. It is local-first and does not upload local files to a server, but malformed binary inputs can still expose parser bugs, denial-of-service behavior, or browser-specific crashes.
...
- whether the issue requires local file input, remote URL input, or hosted samples
