...
test("source HTML has required controls, tabs, and no external runtime assets after build", () => {
  ...
  assert.match(sourceCss, /--frame-i:\s*oklch\(0\.53 0\.21 145\);/);
  assert.match(sourceCss, /--frame-p:\s*oklch\(0\.52 0\.24 260\);/);
  assert.match(sourceCss, /--frame-b:\s*oklch\(0\.58 0\.25 325\);/);
  ...
  assert.match(sourceCss, /\.pill\.i,[\s\S]*?\.pill\.b\s*\{[\s\S]*?color:\s*#fff;/);
  assert.match(sourceCss, /\.pill\.i\s*\{\s*border-color:\s*var\(--frame-i-border\);\s*background:\s*var\(--frame-i\);/);
  assert.match(sourceCss, /\.pill\.p\s*\{\s*border-color:\s*var\(--frame-p-border\);\s*background:\s*var\(--frame-p\);/);
  assert.match(sourceCss, /\.pill\.b\s*\{\s*border-color:\s*var\(--frame-b-border\);\s*background:\s*var\(--frame-b\);/);
